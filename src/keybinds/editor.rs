...
pub fn valid(editor: &mut Editor, press: KeyEvents) {
  ...
  match editor.mode {
    Mode::Selection if press.code == KeyCode::Char('e') || press.code == KeyCode::Char('E') => {
      ...
      match press.modifiers {
        ...
        _ => {
          ...
          match press.code {
            ...
            KeyCode::Up if editor.page_down != 0 && (editor.mode == Mode::Insert || editor.mode == Mode::Selection) => {
              editor.page_down -= 1;
              if editor.cursor.1 - editor.page_down > terminal::get_size().1 as usize - 4 {
                editor.cursor.1 -= 1;
              }
            ...
            KeyCode::Down if (editor.mode == Mode::Insert || editor.mode == Mode::Selection) && (editor.cursor.1 - editor.page_down) < terminal::get_size().1 as usize - 4 => {
              let lines: Vec<&str> = editor.content.lines().collect();
              if lines.len() > 0 && (lines.len() - 1) > editor.cursor.1 {
                editor.cursor.1 += 1;
                let line: String = lines[editor.cursor.1].to_string();
            ...
            KeyCode::Left if editor.mode == Mode::Insert || editor.mode == Mode::Selection => {
              if editor.cursor.0 == 0 {
                if editor.cursor.1 == 0 {}
                else {
                  editor.cursor.1 -= 1;
            ...
            KeyCode::Right if (editor.mode == Mode::Insert || editor.mode == Mode::Selection) && editor.content.len() > 0 => {
              let lines: Vec<&str> = editor.content.lines().collect();
              let line: String = lines[editor.cursor.1].to_string();
              if line.len() > editor.cursor.0 {
                editor.cursor.0 += 1
